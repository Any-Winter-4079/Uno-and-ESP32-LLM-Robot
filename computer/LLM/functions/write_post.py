"""
Post Writer Utility
Saves generated post content as JavaScript modules with JSON data.
Features:
- JSON validation before saving
- Proper JavaScript module export format
- Error handling for invalid JSON
- Configurable output directory
- Standardized file format for web application consumption
"""
...
def write_post(file_name, post_content):
    """
    Write the generated post content to a JavaScript file in module format.
    
    Args:
        file_name: Target JavaScript filename (e.g., "structure-of-the-sun.js")
        post_content: JSON string containing the post data
        
    Returns:
        None - outputs file to disk and prints status message
    """
    try:
        # Validate the JSON structure before writing
        json.loads(post_content)

        # Construct the full file path
        file_path = os.path.join(POSTS_DIR, file_name)

        # Write content as a JavaScript module that exports the JSON
